...
% VENTRAL (Y+) (INCREASING Y's) complexity for the network.

function [BD, VC, ATL, S]=vent_comp(arcs, nodes, newNetwork)
...
for i=1:N
  ...
  % -------------------------------------------------------------
    S(i,3) = nodes(i,3); % Y COORD OF NODE. VENTRAL COMPLEXITY.
...
for i=N+1:N+M
  ...
  % CHANGE THE FOLLOWING LINE TO CHANGE FILTRATION DIRECTION
  % -------------------------------------------------------------
    S(i, 3)=max([nodes(tnRow,3), nodes(bnRow,3)]); % MAX Y COORD OF 2 NODES. VENTRAL COMPLEXITY.
end


[~,BD] = rca1mfscm(S,d);

% Compute VENTRAL complexity=VC
VC=0;
for i=1:size(BD,1)
    if BD(i,2)~=-1
        VC=VC+BD(i,2)-BD(i,1);
