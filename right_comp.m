...
% RIGHT (X+) (looks like increasing x's, but actually DECREASING x's)
% complexity for the network.

function [BD, RC, ATL, S]=right_comp(arcs, nodes, newNetwork)
...
for i=1:N
  ...
  % -------------------------------------------------------------
    S(i,3) = maxX - nodes(i,2); % maxX - X COORD OF NODE. RIGHT COMPLEXITY.
...
for i=N+1:N+M
  ...
  % CHANGE THE FOLLOWING LINE TO CHANGE FILTRATION DIRECTION
  % -------------------------------------------------------------
    S(i, 3)=max([maxX - nodes(tnRow,2), maxX - nodes(bnRow,2)]); % MAX OF maxX - X COORD OF 2 NODES. RIGHT COMPLEXITY.
end


[~,BD] = rca1mfscm(S,d);

% Compute right complexity=RC
RC=0;
for i=1:size(BD,1)
    if BD(i,2)~=-1
        RC=RC+BD(i,2)-BD(i,1);
