...
% LEFT (X-) (looks like decreasing X's, but actually INCREASING X's)
% complexity for the network.

function [BD, LC, ATL, S]=left_comp(arcs, nodes, newNetwork)
...
for i=1:N
  ...
  % -------------------------------------------------------------
    S(i,3) = nodes(i,2); % X COORD OF NODE. LEFT COMPLEXITY.
...
for i=N+1:N+M
  ...
  % CHANGE THE FOLLOWING LINE TO CHANGE FILTRATION DIRECTION
  % -------------------------------------------------------------
    S(i, 3)=max([nodes(tnRow,2), nodes(bnRow,2)]); % MAX X COORD OF 2 NODES. LEFT COMPLEXITY.
end


[~,BD] = rca1mfscm(S,d);

% Compute left complexity = LC
LC=0;
for i=1:size(BD,1)
    if BD(i,2)~=-1
        LC=LC+BD(i,2)-BD(i,1);
