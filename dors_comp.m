...
% DORSAL (Y-) (DECREASING y's)
% complexity for the network.

function [BD, DC, ATL, S]=dors_comp(arcs, nodes, newNetwork)
...
for i=1:N
  ...
  % -------------------------------------------------------------
    S(i,3) = maxY - nodes(i,3); % maxY - Y COORD OF NODE. DORSAL COMPLEXITY.
...
for i=N+1:N+M
  ...
  % CHANGE THE FOLLOWING LINE TO CHANGE FILTRATION DIRECTION
  % -------------------------------------------------------------
    S(i, 3)=max([maxY - nodes(tnRow,3), maxY - nodes(bnRow,3)]); % MAX OF maxY - Y COORD OF 2 NODES. DORSAL COMPLEXITY.
end


[~,BD] = rca1mfscm(S,d);

% Compute dorsal complexity = DC
DC=0;
for i=1:size(BD,1)
    if BD(i,2)~=-1
        DC=DC+BD(i,2)-BD(i,1);
